...
- Added in-`src/` unit tests across `contact.rs`, `error.rs`, `properties.rs`, and `constants.rs` (Tier 2 quality polish), providing fast `cargo test --lib` fail-fast signal alongside the existing integration tests under `tests/`.
